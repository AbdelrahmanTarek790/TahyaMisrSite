...
class JoinRequestCubit extends Cubit<JoinRequestState> {
  ...
  Future<void> createJoinRequest(JoinRequestCreateRequest request) async {
    emit(const JoinRequestState.loading());

    final result = await repository.createJoinRequest(request);

    result.fold(
      (failure) => emit(JoinRequestState.error(message: failure.message)),
      (joinRequest) => emit(JoinRequestState.created(joinRequest: joinRequest)),
    );
  }
  ...
  Future<void> approveJoinRequest(String id, String notes) async {
    ...
    result.fold(
      (failure) => emit(JoinRequestState.error(message: failure.message)),
      (joinRequest) => emit(JoinRequestState.actionCompleted(
