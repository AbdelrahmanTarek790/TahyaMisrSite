...
/**
 * Export event guest registrations to Excel
 * @param {Array} guests - Array of guest objects
 * @param {string} eventTitle - Title of the event
 */
export const exportEventGuestsToExcel = (guests, eventTitle) => {
    const formattedData = guests.map((g) => ({
        الاسم: g.name,
        "البريد الإلكتروني": g.email,
        "رقم الهاتف": g.phone || "N/A",
        المحافظة: g.governorate || "N/A",
        "الرقم القومي": g.nationalId || "N/A",
        "تاريخ التسجيل": g.createdAt ? new Date(g.createdAt).toLocaleDateString("ar-EG") : "N/A",
    }))
    const filename = `${eventTitle.replace(/[^\w\s]/gi, "")}_guest_registrations`
    return exportToExcel(formattedData, filename, "التسجيلات كضيوف")
}
